...
import { getScieloArticleById } from "@/lib/api-sources/scielo";
...
export async function GET(
  ...
  const id = params.id;
  const cacheKey = `scielo:article:${id}`;
  ...
  if (cachedArticle) {
    console.log(`Artigo ${id} encontrado no cache do SciELO`);
  ...
  try {
    ...
    return await requestLimiter.run(async () => {
      ...
      const article = await getScieloArticleById(id);

      if (!article) {
        console.log("Artigo não encontrado no SciELO");
      ...
      console.log(`API SciELO: Artigo encontrado: ${article.title}`);
      return NextResponse.json({ article });
    });
  } catch (error) {
    console.error("Erro ao buscar artigo por ID no SciELO:", error);
    return NextResponse.json(
      {
