...
export function SearchResults({ articles, query, type, language, year, sort }: SearchResultsProps) {
    ...
    <div className="space-y-6">
      {articles.map((article) => (
        <Card key={article.id} className="overflow-hidden transition-all hover:shadow-md">
          ...
          <CardContent>
            <p className="text-slate-600 dark:text-slate-400 line-clamp-3">{article.abstract}</p>
          </CardContent>
          <CardFooter className="flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50 border-t">
            ...
            <div className="flex gap-2">
              <Button
                ...
                aria-label={savedArticles.includes(article.id) ? "Remover dos salvos" : "Salvar artigo"}
