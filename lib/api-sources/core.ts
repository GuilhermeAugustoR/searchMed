...
export async function searchCore(
  ...
  try {
    const { page = 1, pageSize = 50, year } = options

    console.log(`[CORE] Buscando artigos com query: ${query}`)

    // Construir a URL para a API do CORE
    let apiUrl = `${CORE_API_BASE}/search/works?q=${encodeURIComponent(query)}&page=${page}&pageSize=${pageSize}`
    ...
    if (!response.ok) {
      console.warn(`[CORE] API retornou status ${response.status}. Retornando array vazio.`)
      return []
    }
    ...
    const articles: Article[] = data.results.map((result: any) => {
      ...
      // Gerar ID único
      const id = `core-${result.id || Math.random().toString(36).substring(2, 15)}`
      ...
      return {
        ...
        content: `<h2>Abstract</h2><p>${result.abstract || "Resumo não disponível"}</p>`,
        keywords,
        references: [],
        doi,
        url: result.downloadUrl || result.sourceFulltextUrls?.[0] || (doi ? `https://doi.org/${doi}` : null),
        source: "CORE",
      }
    })
...
export async function getCoreArticleById(id: string): Promise<Article | null> {
  try {
    ...
    if (!response.ok) {
      console.warn(`[CORE] API retornou status ${response.status}. Retornando null.`)
      return null
    }
    ...
    const content = `
      ...
      ${result.downloadUrl ? `<h2>Texto Completo</h2><p><a href="${result.downloadUrl}" target="_blank" rel="noopener noreferrer">Download do texto completo</a></p>` : ""}
      ${result.sourceFulltextUrls && result.sourceFulltextUrls.length > 0 ? `<p><a href="${result.sourceFulltextUrls[0]}" target="_blank" rel="noopener noreferrer">Acessar na fonte original</a></p>` : ""}
    `
